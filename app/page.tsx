...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-orange-50/30 text-slate-900 font-sans pb-12">
      {/* Header */}
      <header className="bg-orange-600 border-b border-orange-700 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white leading-tight">ระบบเฝ้าระวังสถานการณ์ฝุ่น PM 2.5</h1>
              <p className="text-xs text-orange-100 font-medium">จังหวัดพิษณุโลก (Phitsanulok PM 2.5 Dashboard)</p>
            </div>
          </div>
          <div className="text-right hidden sm:block">
            <p className="text-sm font-semibold text-white">อัปเดตล่าสุด</p>
            <p className="text-xs text-orange-100">27 มีนาคม 2026, 18:30 น.</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 flex flex-col">
            ...
            <div className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-orange-700 bg-orange-100 px-2.5 py-1 rounded-full w-fit">
              <AlertTriangle className="w-3.5 h-3.5" />
              มีผลกระทบต่อสุขภาพ
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 flex flex-col">
            <div className="flex items-center justify-between mb-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 flex flex-col">
            <div className="flex items-center justify-between mb-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 flex flex-col">
            <div className="flex items-center justify-between mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-orange-50/50 text-orange-800 text-xs uppercase tracking-wider">
                    <th className="p-4 font-semibold border-b border-orange-100">อำเภอ</th>
                    <th className="p-4 font-semibold border-b border-orange-100 text-right">PM 2.5 (µg/m³)</th>
                    <th className="p-4 font-semibold border-b border-orange-100 text-center">ระดับ AQI</th>
                    <th className="p-4 font-semibold border-b border-orange-100 text-right">ประชากรที่กระทบ (คน)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-orange-50">
                  {districtData.map((district) => (
                    <tr key={district.id} className="hover:bg-orange-50/30 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="bg-white rounded-2xl shadow-sm border border-orange-100 flex flex-col">
            ...
            <div className="p-6 flex-1 min-h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={districtData} layout="vertical" margin={{ top: 0, right: 20, left: 20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#ffedd5" />
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} width={80} />
                  <Tooltip 
                    cursor={{ fill: '#fff7ed' }}
                    contentStyle={{ borderRadius: '8px', border: '1px solid #ffedd5', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
          <div className="p-6 border-b border-orange-100 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-orange-100 border-b border-orange-100">
            {activitiesData.map((item, index) => (
              <div key={item.id} className={`p-6 ${index >= 3 ? 'border-t border-orange-100' : ''}`}>
